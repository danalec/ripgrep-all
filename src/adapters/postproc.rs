...
impl FileAdapter for PostprocPrefix {
    async fn adapt(
        ...
        let read = add_newline(postproc_prefix(
            &a.line_prefix,
            postproc_encoding(&a.line_prefix, a.inp, &marker, a.config.text).await?,
        ));
...
impl FileAdapter for PostprocPageBreaks {
    async fn adapt(
        ...
        let read = postproc_pagebreaks(postproc_encoding(&a.line_prefix, a.inp, &marker, a.config.text).await?, prefix, include_empty);
        // keep adapt info (filename etc) except replace inp
        let ai = AdaptInfo {
            inp: Box::pin(read),
...
",
    ...
    async fn test_from_bytes(
        ...
        let inp = postproc_encoding("", inp, "[rga: binary data]", false).await?;
        if pagebreaks {
    ...
    async fn test_binary_content_allowed() -> Result<()> {
        ...
        let inp = postproc_encoding("", inp, "[rga: binary data]", true).await?;
